...
export type PDPProduct = {
  ...
  rating: number;
  reviews: number;
  stock: { qty: number; label: string; leadTime: string };
  images: string[];
  variants: { color: ColorVariant[]; size: SizeVariant[] };
  priceTiers: PriceTier[];
};

export type PDPClientProps = {
  product: PDPProduct;
  related: ProductMock[];
};
...
export default function PDPClient({ product, related }: PDPClientProps) {
  ...
  const [color, setColor] = useState(product.variants.color[0]?.k ?? '');
      ...
      <main id="main" style={{ background: 'var(--color-dark-base-primary)' }}>
        ...
        <div style={{ maxWidth: 1440, margin: '0 auto', padding: '20px 40px' }}>
          <div className="text-mono-xs" style={{ color: 'var(--color-base-500)', display: 'flex', gap: 8 }}>
            ...
            <span style={{ color: 'var(--color-light-base-primary)' }}>{product.name}</span>
        ...
        <section data-screen-label="01 PDP gallery+info" style={{ maxWidth: 1440, margin: '0 auto', padding: '8px 40px 40px', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 40 }}>
          <Gallery images={product.images} cur={img} setCur={setImg} />
          <InfoRail
            product={product}
        ...
        <section data-screen-label="02 PDP tabs" style={{ maxWidth: 1440, margin: '0 auto', padding: '56px 40px' }}>
          <div style={{ borderBottom: '1px solid var(--color-base-800)', display: 'flex', gap: 28, marginBottom: 32 }}>
            {[
              ...
              { k: 'faq',     label: 'FAQ' },
              { k: 'reviews', label: `Avaliações (${product.reviews})` },
            ].map((t) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 20, alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div className="text-mono-xs" style={{ color: 'var(--color-base-500)' }}>
              Escalão {tier?.min ?? 1}+ {(tier?.discount ?? 0) > 0 && <span style={{ color: 'var(--color-accent-100)' }}>· −{tier?.discount}%</span>}
            </div>
